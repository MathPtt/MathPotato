...
pub trait InfixNodeStorageApiAddNodeIdAndTypeToLeft: InfixNodeStorageApi {
    ...
    /// - `left_node_id` - `Uuid`, the node id which is going to be the left child of the
    /// designated target node.
    /// - `left_node_type` - `Uuid`, the node type of the node going to be the child of the
    /// designated target node.
    ///
    /// # Returns
    /// - `Ok(Uuid)` - which is the `target_node_id` when theh operation is successful.
    /// - `Err(ParseError)` - with the details of the error happened during execution.
