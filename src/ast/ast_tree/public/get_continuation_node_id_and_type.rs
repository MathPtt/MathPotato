...
pub trait ContinuationNodeStorageApiGetIdAndType: AstApi {
    /// Returns the `GetContinuationNodeIdAndTypeResult` struct containing the
    /// `id` and `node_type` values of the AST continuation node.
    ///
    /// # Returns
    /// - `Some(GetContinuationNodeIdAndTypeResult)` - The `id` and `node_type`
    ///   is wrapped into a
