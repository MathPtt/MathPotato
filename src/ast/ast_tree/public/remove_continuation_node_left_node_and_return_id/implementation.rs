        ...
        match self.continuation_node.get_type() {
            AstNodeType::InfixOperationAstNode => {
                let continuation_node_id = self.continuation_node.get_id()
                    .unwrap_or_else(||ParseError::new(format!("Error happened while requesting continuation node id.")));
                let id = self.infix_nodes.remove_left_and_return_its_id(continuation_node_id)
                    .unwrap_or_else(|e|ParseError::new(format!("Error happened while removing left node from continuation node. Details: {}", e)));
            },
            _ => Err(ParseError::new(format!("Continuation node is not a type which has left and right side. Continuation node details: {}", self.continuation_node)))
        }
    }
}
